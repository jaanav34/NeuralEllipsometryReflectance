...
if __name__ == "__main__":
    ...
    np.savez(ensure_parent_dir(artifact_path("data", "spectra_norm.npz")), mean=X_mean, std=X_std)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), ensure_parent_dir(artifact_path("models", "spectranet.pt")))
        else:
    ...
    # Load best weights
    model.load_state_dict(torch.load(artifact_path("models", "spectranet.pt"), weights_only=True))
    model.eval()
    ...
    fig.tight_layout()
    out_path = ensure_parent_dir(artifact_path("figures", "loss_curve.png"))
    ...
    fig.tight_layout()
    out_path = ensure_parent_dir(artifact_path("figures", "parity_plot.png"))
