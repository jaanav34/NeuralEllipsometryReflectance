...
# ──────────────────────────────────────────────
# Model (importable without triggering training)
# ──────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    data = np.load(artifact_path("data", "dataset.npz"))
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), ensure_parent_dir(artifact_path("models", "spectranet_v2.pt")))
        else:
    ...
    model.load_state_dict(torch.load(artifact_path("models", "spectranet_v2.pt"), weights_only=True))
    pred_v2, true_v2, mae_v2, r2_per_v2, r2_all_v2 = evaluate_model(
        model, test_loader, "V2"
    )

    # Evaluate V1 on the same test split
    from src.spectranet import SpectraNet

    model_v1 = SpectraNet().to(device)
    model_v1.load_state_dict(torch.load(artifact_path("models", "spectranet.pt"), weights_only=True))
    ...
    fig.tight_layout()
    out_path = ensure_parent_dir(artifact_path("figures", "loss_curve_v2.png"))
    ...
    fig.suptitle("SpectraNetV2 — Predicted vs True (Test Set)", fontsize=13,
                 y=1.02)
    fig.tight_layout()
    out_path = ensure_parent_dir(artifact_path("figures", "parity_plot_v2.png"))
