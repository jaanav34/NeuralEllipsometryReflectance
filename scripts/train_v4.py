...
from src.tmm_simulator import simulate_reflectance_torch
...
if __name__ == "__main__":
    ...
    np.savez(ensure_parent_dir(artifact_path("data", "spectra_norm_v4.npz")), mean=X_mean, std=X_std)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), ensure_parent_dir(artifact_path("models", "spectranet_v4.pt")))
        else:
    ...
    # V4 evaluation
    model.load_state_dict(torch.load(artifact_path("models", "spectranet_v4.pt"), weights_only=True))
    ...
    model_v3.load_state_dict(torch.load(artifact_path("models", "spectranet_v3.pt"), weights_only=True))
    ...
    fig.tight_layout()
    out_path = ensure_parent_dir(artifact_path("figures", "loss_curve_v4.png"))
    ...
    fig.suptitle("SpectraNet V4 — Predicted vs True (Test Set)", fontsize=13,
                 y=1.02)
    fig.tight_layout()
    out_path = ensure_parent_dir(artifact_path("figures", "parity_plot_v4.png"))
